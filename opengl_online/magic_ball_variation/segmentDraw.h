...
void drawCylinderSegment(float height, float radius, int segments)
{
    ...
    for (int i = 0; i < segments + 1; i++)
    {
        float theta = -offset / 2 + i * offset / segments;
         ...
         points[i].x = radius * cos(theta);   // un comment for cylinder
         points[i].y = radius * sin(theta);

        //points[i].x = radius; // uncomment for rectangle
        //points[i].y = sqrt(1 - points[i].x * points[i].x);
    }
...
void drawSphereQuad(float radius, int segments)
{
    ...
    for (i = 0; i <= segments; i++)
    {
        ...
        for (j = 0; j <= segments; j++)
        {
            ...
            points[i][j] = points[i][j].normalize();  // un comment this line to get a curved surface
